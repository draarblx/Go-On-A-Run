...
local gameInfo = { -- Stores all the memory and information for the game.
    ...
    Sounds = { -- This table stores all the static sounds (sounds which are meant to be played once).
        deathSound = love.audio.newSource("sounds/death.mp3", "static"),
        gameOverTheme = love.audio.newSource("sounds/gameOverTheme.mp3", "static")
    },
    BackgroundMusic = love.audio.newSource("sounds/backgroundMusic.mp3", "stream"), -- This is the background music, it is looped and replayed.

    PlayersPlaying = {}, -- Stores all the current players playing.
    LastPlayerCount = 0,
    WorldScrollSpeed = 300, -- Background move speed.
    ...
    Difficulty = {
        Sections = { -- All difficulty data.
            { name = "Easy", speed = 350, obstacleSpeed = 400, spawnDelay = 2.0, spinMultiplier = 1.2, count = 1, img = "assets/Easy.png" },
            { name = "Medium", speed = 425, obstacleSpeed = 500, spawnDelay = 1.5, spinMultiplier = 1.5, count = 1.1, img = "assets/Medium.png" },
            { name = "Hard", speed = 575, obstacleSpeed = 650, spawnDelay = 1.2, spinMultiplier = 2.0, count = 1.25, img = "assets/Hard.png" },
            { name = "Extreme", speed = 650, obstacleSpeed = 800, spawnDelay = 1.0, spinMultiplier = 2.5, count = 1.5, img = "assets/Extreme.png" },
        },
        CurrentIndex = 1,
        Timer = 0,
...
local function CreatePlayerImage(idx)
    ...
    data.Quads = {}
    data.Dead = false
    local success, sheet = pcall(function() return love.graphics.newImage(data.ImgID) end) -- Pcall (protected call) ensures that if an error occurs, the entire code will not stop running.
...
-- All the game's window scaling information to be used by the rest of the script.
local virtualWidth, virtualHeight = 1700, 600
local windowWidth, windowHeight = 1700, 600
local scale, offsetX, offsetY = 1, 0, 0

-- This function is the game's intial load.
function love.load()
    local screenWidth, screenHeight = love.window.getDesktopDimensions()
    screenWidth = screenWidth * 0.885
    local formattedHeight = screenWidth * (600 / 1700)
    love.window.setTitle("Go On A Run")

    love.window.setMode(screenWidth, formattedHeight, { resizable = true, fullscreen = false })
    love.graphics.setBackgroundColor(0.8, 0.9, 1)

    love.resize(screenWidth, formattedHeight)

    love.graphics.setBackgroundColor(0.8, 0.9, 1)

    gameInfo.BackgroundMusic:setLooping(true)
    gameInfo.BackgroundMusic:setVolume(0.08)
    ...
    gameInfo.Background.Scale = formattedHeight / bh
    -- Apply the same scale to both dimensions
    gameInfo.Background.Width = bw * gameInfo.Background.Scale
    gameInfo.Background.Height = formattedHeight  -- This is bh * gameInfo.Background.Scale
...
function love.update(dt)
        ...
        if player and not player.Dead then
            ...
            for _, obs in ipairs(gameInfo.Obstacles) do
                ...
                if hitX < oX + oW and hitX + hitW > oX and hitY < oY + oH and hitY + hitH > oY then -- This line checks if a player is touching the obstacle's hitbox, and will set the player as dead if they are.
                    player.Dead = true
                    gameInfo.Sounds.deathSound:play()
                end
            end
        else
            player.Position.x = player.Position.x - gameInfo.WorldScrollSpeed * dt

            if player.Position.y < gameInfo.PlayerConfig.GroundY then
                player.Velocity.y = (player.Velocity.y or 0) + gameInfo.PlayerConfig.Gravity * dt
                player.Position.y = player.Position.y + player.Velocity.y * dt
                if player.Position.y >= gameInfo.PlayerConfig.GroundY then
                    player.Position.y = gameInfo.PlayerConfig.GroundY
                    player.Velocity.y = 0
    ...
    if aliveCount == 0 and gameInfo.GameState == "playing" then
        ...
        gameInfo.Sounds.gameOverTheme:play()
...
function love.mousepressed(x, y, button)
    ...
    -- Convert mouse coords only if we are in gameplay (scaled area).
    x = x / scale
    y = (y - offsetY) / scale
    ...
    elseif gameInfo.GameState == "select_players" then
        ...
        for _, btn in ipairs(gameInfo.PlayerSelectButtons) do
            if x >= btn.x and x <= btn.x + btn.width and y >= btn.y and y <= btn.y + btn.height then
                ...
                gameInfo.Sounds.gameOverTheme:stop()
...
function love.draw()
    -- These three graphics functions are used to scale the game.
    love.graphics.push()
    love.graphics.translate(0, offsetY)
    love.graphics.scale(scale, scale)
    --[[

    You may see at return statements or at the end of the code, there is this pop function.
    This is use to 'release' this scale feature for the next frame.
    ...
    elseif gameInfo.GameState == "select_players" then
        ...
        love.graphics.pop() -- Example of the pop function.
        return
    elseif gameInfo.GameState == "gameover" then
        ...
        if logo then
            local scale = 0.2
            love.graphics.draw(logo, 850 - (logo:getWidth() * scale) / 2, 60, 0, scale, scale)
        else
            love.graphics.printf("GAME OVER", 0, 100, winWidth, "center")
        end
   ...
   local bg = gameInfo.Background
    ...
    local leftMargin = 20 * bg.Scale
    local bottomMargin = 30 * bg.Scale

    love.graphics.print("High Score: " .. math.floor(gameInfo.HighScore), leftMargin, bg.Height - 90 * bg.Scale)
    love.graphics.print("Score: " .. math.floor(gameInfo.Score), leftMargin, bg.Height - 60 * bg.Scale)
    love.graphics.print("Time in section: " .. math.floor(gameInfo.Difficulty.Timer) .. "s", leftMargin, bg.Height - bottomMargin)
